...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    используя готовые функции из EDA-ядра.

    Возвращает JSON с флагами из compute_quality_flags.
    """
    start = perf_counter()
    ...
    # Фильтруем только булевы флаги для основного вывода
    bool_flags = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
    ...
    print(
        ...
        f"flags_count={len(bool_flags)} latency_ms={latency_ms:.1f} ms"
    )
    ...
    response = {
        ...
        "dataset_info": {
            ...
            "numeric_cols": sum(1 for col in summary.columns if col.is_numeric),
            "categorical_cols": sum(1 for col in summary.columns if not col.is_numeric),
        },
        "latency_ms": latency_ms,
    }
